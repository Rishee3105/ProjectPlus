...
// const getParticularUserRequestStatus = async (req, res) => {
//   try {
//     const userId = req.userId; // The user ID from the request (usually set by a middleware)
//     const projectId = parseInt(req.query.projectId, 10); // Project ID passed as a query parameter
...
//     // Fetch the request status from the 'prequest' table using Prisma
//     const request = await prisma.prequest.findUnique({
//       where: {
//         userId_projectId: {
//           userId,
...
//       return res
//         .status(404)
//         .json({ message: "Request not found for this user and project." });
//     }

//     // If request found, send the status
//     return res.status(200).json({
//       status: request.status,
...
// const getParticularProjectRequests = async (req, res) => {
//   try {
//     const projectId = parseInt(req.query.projectId, 10);
...
//     const requests = await prisma.prequest.findMany({
//       where: {
//         projectId,
//       },
//     });
...
const showPrequestForParticularProject = async (req, res) => {
  try {
    ...
    // Fetch all requests for the project + user info
    const projectRequests = await prisma.prequest.findMany({
    ...
    if (!projectRequests || projectRequests.length === 0) {
      return res
        .status(404)
        .json({ message: "No requests found for this project." });
    }

    // Transform the response
    const requestsWithUserData = projectRequests.map((request) => ({
      id: request.user.id, // User id
      requestId: request.id, // Request id
      name: `${request.user.firstName} ${request.user.lastName}`, // Full name
      charusatId: request.user.charusatId || "N/A",
      profilePhoto: request.user.profilePhoto || null,
      projectName: project.pname,
      projectId: project.id,
      status: request.status.toLowerCase(), // Make status lowercase
    }));
